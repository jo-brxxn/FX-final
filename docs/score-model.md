...
| Trend 1D + 4H (seit v16, 2026-09-25) | Schluss der letzten GESCHLOSSENEN Kerze gegen EMA20; innerhalb ±0,25 × ATR14 → 0, darüber +0,75, darunter −0,75, je Zeitebene; Summe max ±1,5. 1D aus `price_data.json` (dieselben Kerzen wie der Chart, heutiger Tag offen), 4H aus `trend_data.json` (Yahoo 1h → 4h-Blöcke UTC 00/04/…, Block > 4 Tage alt → Zeile fehlt) | COT Data | `applyTrendFeed`, `trendWerte` |
