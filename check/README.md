...
| `logo.js` | **FX-Logo** (2026-09-24). Nutzer: animiertes Logo ("Kerzen wachsen") als Ladesymbol, danach langsam ausblenden; in leeren/sehr leeren Karten unten mittig, Groesse nach freier Flaeche; Cockpit-Intro, Gleiter und Einstellungen weg. Geprueft: kein Intro/Schalter/Gleiter-Feld; Ladebildschirm mit 4 Kerzen, blendet aus und ist dann weg (unter webdriver sofort weg); leere "Pinned notes" mit Logo unten mittig, ohne Inhalt oder Fuss zu ueberdecken; volle Price-Karte ohne Logo; nach Aufklappen eines Indikator-Charts bekommt eine gestreckte Nachbarkarte ein Logo. Gegenprobe `--gegenprobe` (Logo per CSS versteckt). | ja |
