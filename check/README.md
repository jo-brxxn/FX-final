...
| `uebergang.js` | **Wisch-Uebergang, Stapel-Panel, History-Karte** (2026-09-23). Schaltet den Wisch per `window.__wischTest` an (alle anderen Waechter fahren ohne). Prueft: Wisch bei Asset- und Seitenwechsel, das offene Panel ist Teil der Kopie (sonst verschwand es schlagartig - gemessen blockierte der Seitenaufbau 0,5-2,2 s), ein ROHER Klick waehrend des Wischs wirkt auf den neuen Inhalt, kein Wisch bei Fenstern und bei ausgeschalteten Animationen, History-Karte ohne Ueberstand und nur senkrecht scrollbar. ⚠ Zwei Lehren aus dem Bau: `p.click()` wartet von selbst, bis nichts mehr darueber liegt - damit faellt eine klickfangende Ebene nie auf; und die Kopie trug die onclick-Handler der alten Seite (jetzt inert). Gegenprobe `--gegenprobe` | ja |
