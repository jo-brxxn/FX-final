...
| `structure.js` | (seit 2026-09-24 auch: gleich viele `<div>` wie `</div>` im statischen Body) doppelte `id`s, woertlich wiederholte HTML-Bloecke, Handler ohne Funktion/ohne window-Bruecke, und **jedes `var(--x)` ohne Rueckfallwert muss irgendwo definiert sein**. ⚠ Letzteres ist eine eigene Fehlerklasse: CSS meldet ein unbekanntes Token NICHT, es wirft die ganze Deklaration weg ("invalid at computed-value time"). Gemessen 2026-09-13: `--card` war an drei Stellen benutzt und nirgends definiert - vier Kartenarten standen auf `rgba(0,0,0,0)`, also voellig durchsichtig, und der Nutzer sah "alles noch so weiss". Der Kontrast-Waechter konnte das nie sehen, der prueft DEFINIERTE Tokens. Geprueft wird nur echtes CSS (`<style>`-Bloecke und `style="..."`), nicht die ganze Datei - sonst stolpert die Pruefung ueber das Wort `var(--x)` im Erklaertext des Versions-Banners. **Seit 2026-09-14 zusaetzlich die einheitliche Restzeit** (Nutzer-Regel: aus *Tomorrow* wird `1d`, heute bekommt ein auffaelliges `!`): keine Restzeit-Beschriftung in `js/*.js` darf wieder "Tomorrow" sagen, `countdownHtml()` muss existieren und `span.cd-heute` setzen, und `.cd-heute` muss in `index.html` formatiert sein - ein `<span>` ohne CSS-Regel waere unsichtbar, also das Gegenteil von auffaellig. ⚠ Die Kette stand an VIER Stellen als eigene Kopie, mit vier Schreibweisen; drei waren von Hand zu finden, die vierte fand erst dieser Waechter. Fliesstext ("check back tomorrow for a trend line") ist erlaubt, geprueft wird nur ein Textstueck, das im Wesentlichen aus dem Wort besteht | nein |
| `symbole.js` | **Flaggen und Kartensymbole** (2026-09-23). Die Flagge war in 4/10 Streifen zerschnitten, die einzeln kippten: Luecken, Stufen, Saegezahn-Kante; der Glanz lief 107 px vor bis 100 px hinter der Flagge; JPY-Weiss verschwand auf dem Grund. Geprueft: jede Flagge EIN Stueck, Glanz/Falten in der Clip-Welle, Rand vorhanden, kein reines Weiss; am PIXEL (dunkler Testgrund - auf dem hellen war die erste Fassung blind), dass neben der Flagge nie Glanz erscheint; jeder sichtbare Kartenkopf auf 11 Seiten traegt genau EIN Symbol; kein Emoji in den Sentiment-Reitern. Gegenprobe `--gegenprobe` (Glanz ohne Clip) **Seit VERSION-CHECK-544 zusaetzlich (D):** keine `<circle>` in USD/EUR/AUD/NZD (Flagge und einfarbiges Symbol) und Sternzahl 50/12/6/4 - vorher standen dort Punkte; die Kopf-Flagge liegt ganz in der Kopfkarte (gemessen vorher 510x340 in 404x113); `#aiRim` traegt `vector-effect` (an `.ai-rim` vererbt es sich nicht durch `<use>`, der Rand wuchs auf 9-37 px). Gegenprobe `--gegenprobe-sterne` (Punkt + Riesenflagge) **Seit 546:** das Kopf-Band `.ahead-motif-band` liegt in der Karte und traegt keine Welle/Glanz/Animation (Gegenprobe: `<animate>` im Band meldet rot). **Seit 547 (E):** jeder Filter am Kopf-Band hat eine feste Flaeche (`filterUnits="userSpaceOnUse"`) innerhalb der Band-viewBox - mit dem Box-Filter `#aiDuo` rechnete WebKit die gestreckte Randspalte ungeschnitten (9747 statt 144 Einheiten, ~91 000 px bei Pixeldichte 2) und zeichnete das Band auf dem iPad gar nicht. Chromium zeigt den Fehler nicht, geprueft wird deshalb die Bauform. Gegenprobe `--gegenprobe-band`. **Seit 550:** das Band traegt gar keinen Filter mehr (gezeichnete Fassung `#ai-<id>-g`, Farben vorab umgerechnet) - geprueft: kein Filter im Band. | ja |
| `uebergang.js` | **Wisch-Uebergang, Stapel-Panel, History-Karte** (2026-09-23). Schaltet den Wisch per `window.__wischTest` an (alle anderen Waechter fahren ohne). Prueft: Wisch bei Asset- und Seitenwechsel, im ersten Bild liegt die ALTE Seite als Originalknoten in #detail obenauf (Titel-Schrift wie vorher, deckt die Flaeche, eigener Hintergrund, hinter der neuen .dp) und ist nach dem Wisch restlos weg (Bugreports 2026-09-23: geklonte Seite verlor ausserhalb ihrer Vorfahren das CSS; der danach eingebaute Vorhang zeigte 340-514 ms nur Weiss), ein ROHER Klick waehrend des Wischs wirkt auf den neuen Inhalt, kein Wisch bei Fenstern und bei ausgeschalteten Animationen, History-Karte ohne Ueberstand und nur senkrecht scrollbar. ⚠ Zwei Lehren aus dem Bau: `p.click()` wartet von selbst, bis nichts mehr darueber liegt - damit faellt eine klickfangende Ebene nie auf; und die fruehere Kopie trug die onclick-Handler der alten Seite. Gegenproben `--gegenprobe` (klickfangende Ebene) und `--gegenprobe-kopie` (alte Seite ausserhalb ihrer Vorfahren, ohne ids) **Seit VERSION-CHECK-545:** die FX-Flagge im Wisch muss ein eigenstaendiges `<img>` sein, dekodieren, jeden `url(#..)`/`href="#.."`-Verweis selbst definieren und ohne Mischmodus/ai-Klassen auskommen (iPad zeichnete die Inline-Variante nicht, Chromium schon - deshalb strukturell geprueft). Gegenprobe `--gegenprobe-flagge` **Seit 546:** Wisch-Flaggenbild ohne `<animate`, Einblendung von Seite/Dashboard-Karte und `WISCH_MS` genau 1 s (Gegenprobe: `--enter-dur:.3s` meldet rot). **Seit 548:** beim Waehrungswechsel ist die alte `.dp` nicht hoeher als die sichtbare Seitenflaeche (vorher volle Inhaltshoehe 2594 px, ~9,5 Mio. Pixel bei Pixeldichte 2 - der Waehrungswechsel hing auf dem iPad). Gegenprobe `--gegenprobe-hoehe`. **Seit 549 (Neubau):** die alte Ansicht beim Waehrungswechsel liegt in einer Huelle (`.body > .detail`) NEBEN `#pgCur`, nie im Scrollbereich `#detail` (dort hing der Wechsel auf dem iPad). Gegenprobe `--gegenprobe-scroll`. **Seit 550:** (a) 1,5 s belegter Hauptthread direkt nach dem Anlegen des Wischs - danach muss die Flagge in >= 15 Bildern sichtbar durchfahren (bis 549: 0 Bilder, die Notbremse raeumte ab - auf dem iPad "es haengt und dann wechselt es ohne Animation"); Gegenprobe `--gegenprobe-start` (Uhr ab dem Anlegen), gegen den echten Stand 549 ebenfalls rot. (b) Flaggenbild ohne SVG-Filter, bewegte Ebene ohne CSS-filter. **Seit 551:** hoechstens 50 `toLocaleDateString`-Aufrufe je Waehrungswechsel (vorher 809 - jeder baut intern einen neuen `Intl.DateTimeFormat`, der Aufbau stand spuerbar still); Gegenprobe `--gegenprobe-tempo`, gegen den Stand 550 rot (805). **Seit 553:** im ersten Bild nach dem Waehrungs-Tap ist das Panel zu und die Waehrung markiert, der Aufbau folgt erst danach (gegen den Stand 552 rot: "Aufbau LIEF SCHON"). | ja |
| `scrollhalt.js` | **Scrollstand beim Neuzeichnen der Asset-Seite** (2026-09-23). Nutzer (iPad): nach einer Aenderung in einer Karte "weiter oben auf der Seite, als ob ich abrupt hochgescrollt habe". In WebKit gemessen: `renderDetail` tauscht `#detail` per `innerHTML` aus, WebKit kuerzt `scrollTop` sofort (Scrollbereich im Moment des Austauschs leer) - Zeitraum-Knoepfe eines Charts 4393 -> 247. Chromium kuerzt erst am Ende und zeigt es nie; der Waechter stellt WebKit nach (erst leeren, Layout lesen, dann fuellen) und klickt jedes Bedienelement der Asset-Seite (USD, GOLD), das auf ihr bleibt. Dazu Matrix, Seasonality, Carry, Calendar per Klick und alle Seiten-Render-Funktionen OHNE Klick (Datenupdates) bei halb gescrollter Seite. Gegen den Stand 551 rot (70 Befunde). Gegenprobe `--gegenprobe` (Zuruecksetzen blockiert). | ja |
| `kalender.js` | **Kalender-Karte der Asset-Seite: vergangene Tage** (2026-09-23). Nutzer: vergangene Tage grau/blass, roter Punkt bleibt, Eintraege mit Actual, blasser; aeltere Tage per Rueckfrage "Aus Indikator-Historie". Vorher waren alle Tage vor dem Feed-Beginn (~3 Tage zurueck) blass OHNE Punkt und hiessen "Not published yet". Geprueft (USD): jeder vergangene Tag "vorbei" + Zahl blass, Punkt voll; der letzte Release-Tag vor dem Feed-Beginn hat einen Punkt und im Tagesfenster Eintraege, deren Actual/Forecast/Previous exakt `ind_data.json` historyFull entsprechen, blass; keine Eintraege ohne Kalender-Zuordnung (Regel 4). Gegen den Stand 552 rot (25 Befunde). Gegenprobe `--gegenprobe` (Blass-Regel per CSS ausgehebelt). | ja |
| `regeln.js` | **Regel-basierte Score-Teile** (2026-09-24). Nutzer-Paket: COT (Netto ab 60 % ±0,75, Wochenaenderung 3/5/7,5/10 Pkt -> 0,75/1,05/1,275/1,35, Summe max ±1,5), Rendite-Trends je 0,75, 2Y-Zinsdifferenz 20 Handelstage (±0,5 ab 10 bp, ±0,75 ab 25 bp), Carry (2Y, Carry-to-Risk ab 0,2/0,4, aus bei VIX +30 %), Rohstoffe AUD/NZD/CAD, CB Tone/Next CB Move weg, Score-Fenster, Momentum-Karte. Zinsdifferenz und Rohstoffe werden in Node unabhaengig aus `bond_data.json`/`commodity_data.json` nachgerechnet. Gegen 560 rot (Abbruch: Regeln fehlen). Gegenprobe `--gegenprobe` (verfaelschte COT-Zeile). | ja |
| `kartentitel.js` | **Kartentitel, ⓘ, angepinnte Notizen** (2026-09-24). Nutzer: Makro-Titel groesser als die uebrigen -> einheitlich "ca. in der Mitte"; ⓘ leicht kleiner als die Schrift daneben und blau; Notizen staerker in der Bias-Farbe, Titel groesser. Vorher: Titel 22/17/15/13px, ⓘ grau 18px neben blau 13px, Notizen grau mit 3px-Strich. Geprueft (Dashboard, Asset EUR, jede Seite): jeder Kartentitel = `--fs-kt` (17px); jedes ⓘ im Kartenkopf blau und schmaler als der Titel; eine bullishe angepinnte Notiz blau getoent, Titel >= 15px. Gegen 557 rot (39 Befunde). Gegenprobe `--gegenprobe`. | ja |
| `logo.js` | **FX-Logo** (2026-09-24). Nutzer: animiertes Logo ("Kerzen wachsen") als Ladesymbol, danach langsam ausblenden; in leeren/sehr leeren Karten unten mittig, Groesse nach freier Flaeche; Cockpit-Intro, Gleiter und Einstellungen weg. Geprueft: kein Intro/Schalter/Gleiter-Feld; Ladebildschirm mit 4 Kerzen in GENAU EINEM Durchlauf (`lgKerzeLade`), beim Ausblenden bleibt jede Kerze voll stehen (vorher begann die Schleife beim Ausblenden neu, 54/57 Bilder), blendet aus und ist dann weg (unter webdriver sofort weg); leere "Pinned notes" mit Logo unten mittig, ohne Inhalt oder Fuss zu ueberdecken; volle Price-Karte ohne Logo; nach Aufklappen eines Indikator-Charts bekommt eine gestreckte Nachbarkarte ein Logo. Gegenprobe `--gegenprobe` (Logo per CSS versteckt). | ja |
